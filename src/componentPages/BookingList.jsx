...
const BookingList = () => {
  ...
  let handleCancelTickets = (bookingID) => {
    ...
    axios.delete(`http://localhost:3000/booking/${booking.id}`)
      .then(() => {
        axios.get(`http://localhost:3000/${booking.type}/${booking.refID}`)
          .then((res) => {
            let updatedSeats = res.data.seats + booking.seats;
            axios.patch(`http://localhost:3000/${booking.type}/${booking.refID}`, {
              seats: updatedSeats
            }).then(() => {
